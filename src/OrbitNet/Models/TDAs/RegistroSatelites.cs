...
namespace OrbitNet.Models.TDAs
{
    ...
    {
        [cite_start]// Puntero raíz del árbol almacenado en el Heap de la RAM [cite: 41, 42]
        ...
        [cite_start]// Calcula el Factor de Equilibrio con la fórmula exacta: FE = H_der - H_izq [cite: 210]
        ...
        // ==========================================================
        [cite_start]// ROTACIONES MANUALES DE PUNTEROS [cite: 53, 212]
        // ==========================================================

        [cite_start]// Rotación Simple a la Derecha (LL) - Inserción en hijo izquierdo del subárbol izquierdo [cite: 214, 217]
        private AvlNode RotacionDerecha(AvlNode y)
        {
            AvlNode x = y.LeftChild;
            AvlNode T2 = x.RightChild;

            [cite_start]// Quirófano de punteros: intercambiamos referencias en memoria RAM [cite: 39]
            x.RightChild = y;
            y.LeftChild = T2;
        ...
        [cite_start]// Rotación Simple a la Izquierda (RR) - Inserción en hijo derecho del subárbol derecho [cite: 213, 218]
        ...
        [cite_start]// Método público expuesto para ser invocado por el motor de ingesta XML [cite: 263, 270]
        ...
        {
            [cite_start]// Base de la recursividad: se encuentra el espacio vacío en el Heap [cite: 42]
            if (nodoActual == null)
            {
            ...
            {
                nodoActual.RightChild = InsertarRecursivo(nodoActual.RightChild, id, nombre, frequency);
            }
            else
            {
                [cite_start]// Regula que no existan duplicados en el catálogo global [cite: 205]
                throw new Exception($"El Satélite con ID '{id}' ya existe en el catálogo global.");
            }

            [cite_start]// Actualizamos la altura del nodo actual tras la inserción de la hoja [cite: 208]
            nodoActual.Height = 1 + Maximo(ObtenerAltura(nodoActual.LeftChild), ObtenerAltura(nodoActual.RightChild));

            [cite_start]// Evaluamos el factor de balanceo riguroso sobre el nodo ancestro [cite: 52, 210]
            int fe = ObtenerFactorEquilibrio(nodoActual);

            // ------------------------------------------------------
            [cite_start]// EVALUACIÓN DE LOS 4 CASOS DE DESBALANCE (|FE| >= 2) [cite: 211, 212]
            // ------------------------------------------------------

            [cite_start]// Caso 1: Rotación Simple Derecha (LL) [cite: 214, 217]
            if (fe < -1 && string.Compare(id, nodoActual.LeftChild.SatelliteId) < 0)
            {
                return RotacionDerecha(nodoActual);
            }

            [cite_start]// Caso 2: Rotación Simple Izquierda (RR) [cite: 213, 218]
            if (fe > 1 && string.Compare(id, nodoActual.RightChild.SatelliteId) > 0)
            {
                return RotacionIzquierda(nodoActual);
            }

            [cite_start]// Caso 3: Rotación Doble Izquierda-Derecha (LR) [cite: 215]
            if (fe < -1 && string.Compare(id, nodoActual.LeftChild.SatelliteId) > 0)
            {
            ...
            [cite_start]// Caso 4: Rotación Doble Derecha-Izquierda (RL) [cite: 216]
            if (fe > 1 && string.Compare(id, nodoActual.RightChild.SatelliteId) < 0)
            {
